...
export class ProductsService {
  ...
  async insert(insertProductDto: InsertProductDto): Promise<string> {
    ...
    const insert = await insertProduct.save();
    return insert.id;
  }
  ...
  async updateById(
    ...
    const response = await this.productModel
      .findByIdAndUpdate(id, insertProductDto, { new: false, upsert: true })
      .exec();
    return response.id;
  }
  ...
  async list(
    ...
    const query = {};
    const cursor = this.productModel.find(query).populate('name');
